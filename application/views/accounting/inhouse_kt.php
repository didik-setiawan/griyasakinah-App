...
<table class="table table-bordered" id="theTable">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <?php if($d->status == 0){ ?>
                    <button <?= $action ?> class="btn btn-xs btn-success toCode" data-id="<?= $d->id_kt ?>" data-type="inhouse_kt"><i class="fa fa-check"></i></button>
                <?php } else if($d->status == 1 || $d->status == 2){ ?>
                    <button class="btn btn-xs btn-primary toBayar" data-id="<?= $d->id_kt ?>" data-type="inhouse_kt" data-sisa="<?= $total_bayar ?>"><i class="fa fa-plus"></i></button>
                <?php } ?>
            </td>
        </tr>
        <?php } ?>
